...
<div class="max-w-lg mx-auto">
    ...
    <div class="bg-surface border border-border rounded-lg p-6">
        <form method="POST" action="{{ route('ppdb.conversations.store') }}" class="space-y-4">
            @csrf
            <div>
                <label for="applicant_id" class="block text-sm font-medium text-text mb-1">Terkait Pendaftaran <span class="text-muted font-normal">(opsional)</span></label>
                <select id="applicant_id" name="applicant_id" class="w-full rounded-md border border-border px-3 py-2 text-sm">
                    <option value="">— Tidak terkait pendaftaran tertentu —</option>
