...
<div class="max-w-xl mx-auto">
    ...
    <h1 class="text-xl font-semibold text-text mt-2">{{ $conversation->subject }}</h1>
    <p class="text-sm text-muted mb-6">dengan {{ $conversation->user->name }}</p>

    <div class="space-y-3 mb-6">
        @foreach ($conversation->messages as $message)
            <div class="{{ $message->sender_side === 'admin' ? 'ml-auto bg-primary/10' : 'mr-auto bg-surface border border-border' }} rounded-lg p-3 max-w-[80%] {{ $message->sender_side === 'admin' ? 'text-right' : '' }}">
    ...
    <form method="POST" action="{{ route('admin.conversations.reply', $conversation) }}" class="flex gap-2">
        @csrf
